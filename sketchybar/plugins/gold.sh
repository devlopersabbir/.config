...
# Fetch Live Gold (PAXG / XAU) Price and 15-minute percentage price change
CACHE_FILE="/tmp/sketchybar_gold_price"

fetch_gold() {
  local raw
  raw=$(curl -s --connect-timeout 1.5 --max-time 2 "https://api.binance.com/api/v3/klines?symbol=PAXGUSDT&interval=15m&limit=1" 2>/dev/null)
